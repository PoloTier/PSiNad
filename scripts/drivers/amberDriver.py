...
def prepare(geometry, ks_config):
    ...
    if not os.path.isfile(ks_config.topo1): Log.fatalError(
        'mandatory input file {0} not found!'.format(ks_config.topo1)
    )

    ## get real sander input from cobram.ks_config
    low_sander_input = ks_config['MM']['AMBER']['level0'].split('\n')
    if len(low_sander_input) == 0:
        Log.fatalError("missing !sander ... ?sander section in the ks_config file" )
    ...
    if geometry.calculationType == 'M' or geometry.calculationType == 'ML':
        #Log.writeLog('No topology check needed: model-H.top is not needed')
        pass
    else:
        ## check the consistency of the atom type in real.top e model-H.top
        if not os.path.isfile(ks_config.topo2): Log.fatalError(
            'mandatory input file {0} not found!'.format(ks_config.topo2))
        checktoptop(geometry,ks_config.topo1,ks_config.topo2)

    ## Read the charge of real from real.top
    CRG_real=read_crg_from_top(ks_config.topo1)
    CRG_model_H = read_crgA()

    if geometry.calculationType == 'HML' or geometry.calculationType == 'HL' or geometry.calculationType == 'ML':
        ## get free mm atoms from cobram.ks_config
        freeMM = []
        try:
            freeMM = ks_config['MM']['AMBER']['freemm']
        except KeyError:
            pass 

        bellymask = []
        try:
            bellymask = ks_config['MM']['AMBER']['bellymask']
        except KeyError:
            pass 
    ...
    cretesander_real(low_sander_input,geometry,ks_config)
...
def MMcalculations(geometry, ks_config):
    ...
    if "L" in geometry.calculationType and ifskip == 0:  # low optimization is not skipped
        sander_opt_low(ks_config) # optimization of low
    ...
    E_real, Fxyz_real = sander_real(geometry, ks_config)
    #####################
    # amber modelnoc MM single point on real w ZERO charges on model-H
    E_real_modelnoc, Fxyz_real_modelnoc = sander_real_modelnoc(ks_config)
    ...
    E_modelH, Fxyz_modelH = sander_modelH(geometry,ks_config)
...
def checktoptop(geometry,realtop1,modelhtop1):
    ...
    if (geometry.NatomQM+geometry.NsubH)!=(len(modelhat)):
        Log.fatalError('the number of QM atoms in model-H.top ('+str(len(modelhat))+
    ...
    for i in range(geometry.NatomQM):
        if (modelhat[i]!=realat[geometry.list_HIGH[i]-1]):
            Log.fatalError('atom types do not correspond in real.top \t'+realat[geometry.list_HIGH[i]-1]+'\n'+
                           '\tand model-H.top \t\t\t\t'+modelhat[i]+' atom '+str(i+1)+'\n'+
                           '\tcheck your input files\n')
    #Log.writeLog('atom type check done!')
    return
...
def free_MM(frozenMM,geometry,low_sander_input,bellymask):
    ...
    if (geometry.calculationType == 'HM'):
        ...
        if (frozenMM!=[] and bellymask!=[]):
            # print(frozenMM)
            # print(bellymask)
            Log.fatalError('It is not possible to define freeMM and bellymask atoms in a single run\n')
        if (frozenMM==[]):
            ...
            sander.write('\n')
            sander.close()
        #Log.writeLog('MM Frozen atoms check done!')
    return new_input[i]

def cretesander_real(low_sander_input,geometry,ks_config):
...
# (OW) Do the pmemd trick, compute one MD step in dielektrikum to obtain velocities for GPU-code
        if ks_config.get('MM.AMBER.type', 'serial')=='gpu':
            low_sander_input= ["minimizzazione","&cntrl","imin = 1,","ncyc = 1,","maxcyc = 0,","ntb = 0,","igb = 1,","extdiel = 1","ntr = 0,","DRMS = 0.0005,","ntxo=1,","/"]
        else:
            low_sander_input= ["minimizzazione","&cntrl","imin = 1,","ncyc = 1,","maxcyc = 0,","ntb = 0,","igb = 0,","ntr = 0,","DRMS = 0.0005,","ntxo=1,","/"]

    ## ks_configs and write a correct second input
    p='belly'
...
#    low_sander_input=filter(lambda low_sander_input:low_sander_input.find(p) == -1,low_sander_input)
    p='maxcyc'
    low_sander_input=list(filter(lambda low_sander_input:low_sander_input.find(p) == -1,low_sander_input))
    if ks_config.get_nested('MM.AMBER.type', 'serial') != '0':
        p='imin'
        low_sander_input=list(filter(lambda low_sander_input:low_sander_input.find(p) == -1,low_sander_input))
    p='/'
    low_sander_input=list(filter(lambda low_sander_input:low_sander_input.find(p) == -1,low_sander_input))
...
# do the pmemd-trick to get forces from velocities (OW)
        # sander.write('  cut = '+ str(ks_config.get_nested('MM.AMBER.cutoff', '999'))+',\n')
        if ks_config.get('MM.AMBER.type', 'serial') != 'serial':
...
def read_crg_from_top(filename):
    ...
    try:
       for i in range(len(crg_real_tmp)):
           ...
           crg_real=[]
           Log.writeLog('Amber version > 11 detected!\n')
...
def checksanderdone(filename, sandertype):
    ...
    if SanderError: Log.fatalError('error termination in '+sandertype+' calculation!')
    return


def sander_opt_low(ks_config):
    ...
    # get the ks_config for sander parallel execution
    para_exe = os.getenv('PARA_EXE')
    if para_exe is None:
       para_exe='mpirun -np'

    Log.writeLog("Perform a full calculation of the entire system ...")

    # run sander
    if ks_config.get('MM.AMBER.type', 'serial')=='1':
        _ks_config = shlex.split( 'pmemd.cuda -O -i real-sander-first.inp -o real-sander-first.out -p real.top -c real.crd -r real.rst' )
    elif ks_config.get('MM.AMBER.type', 'serial')=='2':
        _ks_config = shlex.split( para_exe+' '+str(ks_config[7])+' sander.MPI -O -i real-sander-first.inp -o real-sander-first.out -p real.top -c real.crd -r real.rst' )
    else:
        _ks_config = shlex.split( 'sander -O -i real-sander-first.inp -o real-sander-first.out -p real.top -c real.crd -r real.rst' )
...
def sander_real(geometry,ks_config):
    ...
    if ks_config.get('MM.AMBER.type', 'serial')=='1':
        _ks_config = shlex.split( 'pmemd.cuda -O -i real-sander-second.inp -o real-sander-second.out -p real.top -c real.rst -r real-second.rst')
    elif ks_config.get('MM.AMBER.type', 'serial')=='2':
        _ks_config = shlex.split(para_exe+' '+str(ks_config[7])+' sander.MPI -O -i real-sander-second.inp -o real-sander-second.out -p real.top -c real.rst -r real-second.rst')
    else:
    ...
    if ks_config.get('MM.AMBER.type', 'serial') != 'serial' :
         ...
         E_sander_real=read_energyMM_out('real-sander-second.out')
    else:
         Fxyz_second=read_forcesA('forcedump.dat')
         E_sander_real=read_energyMM()

    if geometry.calculationType == 'M' or geometry.calculationType == 'ML':
        Log.writeLog(' done!\n')
        Log.writeLog('MM energy is {0} Hartree\n'.format(E_sander_real))

    return [E_sander_real,Fxyz_second]


def sander_real_modelnoc(ks_config):
    ...
    # run sander
    if ks_config.get('MM.AMBER.type', 'serial')=='1':
        _ks_config = shlex.split('pmemd.cuda -O -i real-sander-second.inp -o real-sander-modelnoc.out -p real-modelnoc.top -c real.rst -r real-modelnoc.rst')
    elif ks_config.get('MM.AMBER.type', 'serial')=='2':
        _ks_config = shlex.split(para_exe+' '+str(ks_config[7])+' sander.MPI -O -i real-sander-second.inp -o real-sander-modelnoc.out -p real-modelnoc.top -c real.rst -r real-modelnoc.rst')
    else:
        _ks_config = shlex.split('sander -O -i real-sander-second.inp -o real-sander-modelnoc.out -p real-modelnoc.top -c real.rst -r real-modelnoc.rst')
    subprocess.call( _ks_config )
...
def sandertotalnoc(ks_config):
    ...
    # run sander
    if ks_config.get('MM.AMBER.type', 'serial')=='1':
        _ks_config = shlex.split('pmemd.cuda -O -i real-sander-second.inp -o real-sander-totalnoc.out -p real-totalnoc.top -c real.rst -r real-totalnoc.rst')
    elif ks_config.get('MM.AMBER.type', 'serial')=='2':
        _ks_config = shlex.split(para_exe+' '+str(ks_config[7])+' sander.MPI -O -i real-sander-second.inp -o real-sander-totalnoc.out -p real-totalnoc.top -c real.rst -r real-totalnoc.rst')
    else:
        _ks_config = shlex.split('sander -O -i real-sander-second.inp -o real-sander-totalnoc.out -p real-totalnoc.top -c real.rst -r real-totalnoc.rst')
    subprocess.call( _ks_config )
...
def sander_modelH(geometry,ks_config):
    ...
    # run sander
    if (ks_config.get('MM.AMBER.type', 'serial'))=='1':
      _ks_config = shlex.split('pmemd.cuda -O -i model-H-sander.inp -o model-H.out -p model-H-noc.top -c model-H.crd -r model-H.rst')
    elif ks_config.get('MM.AMBER.type', 'serial')=='2': ## model-H cannot be run in parallel, there is only 1 residue!
...
def save_MM_amber_step(step,ks_config):
    ...
    if not os.path.exists(logName):
        Log.writewarning( "Amber file {0} cannot be found: the {1} file will not be updated".format(logName, allName) )
...
def read_forcesA_pmemd(filename):
    ...
    for i in range(len(atom)):
       if atom[i]=='C':
           ...
           Log.fatalError("Atom "+atom[i]+" not implemented in forces_pmemd routine, please change the source code!")
    for i in range (len(atom)):
...
def skip_MMfirst(step, ks_config, cobcom):
    # ks_config=lists[6]
    ifskip=0
    if ks_config[1] == 'freqxg':
        ifskip=1  # low optimization is skipped
        Log.writeLog('This is a frequency run: the initial full MM calculation is skipped\n')
    elif step==0:
        ifskip=1  # low optimization is skipped
        Log.writeLog('This is the 0th step: the initial full MM calculation is skipped\n')
    else:
        #Log.writeLog('Cobram is looking for some coordinates to be numerically differentiated\n')
        Dnumber=0
        ...
        for i in range(len(frozenQM)):
            ...
            if splitted[-1] in ['d','D']:
                Dnumber=Dnumber+1
        if Dnumber!=0:
            Log.writeLog("Numerical 2nd derivative is requested for "+str(Dnumber)+" coordinates\n")
            Log.writeLog('so for the first '+str(Dnumber)+' steps the low layer is not optimized\n')
            if step<Dnumber+1:
                Log.writeLog('step= '+str(step)+': is in the range [1 , '+str(Dnumber)+']\n')
                Log.writeLog('low layer is NOT optimized\n')
                ifskip=1
            else:
                Log.writeLog('step= '+str(step)+': is out of the range [1 , '+str(Dnumber)+']\n')
                Log.writeLog('low layer is optimized\n')
                ifskip=0
    return ifskip
