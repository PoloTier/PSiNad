    ...
    for line in proc.stdout:
        ...
        try:
            if key not in ["BASH_FUNC_module%%", "BASH_FUNC_ml%%", "LS_COLORS"]:
                ...
                PSND_SCRIPTS_ENV[key] = value.rstrip('\n')
        except ValueError:
...
def findPSNDProfile() -> Optional[str]:
    """
...
def checkPSNDProfile(filenameconf: str) -> str:
    """
    @brief Checks if the environment for PSND SCRIPTS execution is properly 
...
def checkPSNDEnv() -> Tuple[bool, str]:
    """
    @brief Checks if the environment for PSND SCRIPTS execution is properly 
    defined and if all the necessary executables are available.

    This function verifies that the environment required for PSND SCRIPTS is 
    ...
    """
    # Check if the environmental variable with the <PSND SCRIPTS> path is defined
    message: str = ""
    try:
        psnd_scripts_path = os.environ['PSND_SCRIPTS_PATH']
        os.environ['PATH'] += os.pathsep + psnd_scripts_path
        message = f'PSND_SCRIPTS_PATH={psnd_scripts_path}, add it to PATH'
    except KeyError:
    ...
    '''
    for exe in ["psndqm.py", "psndqmmm.py"]:
        if not which(exe):
...
def getVersion():
    ...
    wordDir = os.getcwd()
    try:
        KS_Path = os.environ["PSND_SCRIPTS_PATH"]
        os.chdir(KS_Path)
    except KeyError: 
        # try in current dir
        pass
...
if __name__ == '__main__':
    ...
    from psnd_arg import QMSolverType, MMSolverType
    ...
    Log.writeLog(f'find profile is: {findPSNDProfile()}\n')
    Log.writeLog(f'source profile info: {checkPSNDProfile(findPSNDProfile())}\n')
    Log.writeLog(f'check environment info: {checkPSNDEnv()}\n')
    for k,v in PSND_SCRIPTS_ENV.items():
        is_third = False
