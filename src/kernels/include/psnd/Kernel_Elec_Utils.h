...
class elec_utils {
   public:
     ...
     */
    static int ker_from_c(psnd_complex* ker, psnd_complex* c, psnd_real xi, psnd_real gamma, int fdim) {
    ...
    static double calc_ElectricalEnergy(psnd_real* E, psnd_complex* wrho, int occ) {
        double Ecalc = 0.0e0;
    ...
    static int calc_distorted_rho(psnd_complex* wrho,   // distorted density
                                  psnd_complex* rho,    // rho_ele
                                  double        xi,     // xi must be 1
                                  double        gamma,  // gamma must be 0
                                  double        alpha) {
        // initialize distorted-density
        psnd_real    L    = 1.0e0 - log(std::abs(alpha));  // @NOTE
        psnd_complex norm = 0.0e0;
    ...
    static int hopping_impulse(psnd_real* direction, psnd_real* np, psnd_real* nm,  //
                               psnd_real Efrom, psnd_real Eto, int from, int to, bool reflect) {
        ...
        psnd_real coeffd = coeffb * coeffb - 4 * coeffc;
        if (coeffd > 0) {
            psnd_real x1 = 0.5e0 * (-coeffb + sqrt(coeffd)), x2 = 0.5e0 * (-coeffb - sqrt(coeffd));
            psnd_real xx = (std::abs(x1) < std::abs(x2)) ? x1 : x2;
    ...
    static int hopping_impulse(psnd_real* direction, psnd_real* np, psnd_real* nm, psnd_real* eig,  //
     ...
     */
    static int c_sphere(psnd_complex* c, int fdim) {
    ...
    static int c_focus(psnd_complex* c, double xi, double gamma, int occ, int fdim) {
    ...
    static int rho_focus(psnd_complex* rho, int iocc, double gamma_ou, double gamma_uu, int fdim, bool rand_act,
    ...
    static int c_window(psnd_complex* c,     // array[fdim]: c = (xele + im*pele) / sqrt(2)
        ...
        switch (type) {
            ...
            case ElectronicSamplingPolicy::SQCspx2: {
                psnd_complex* cadd1 = new psnd_complex[Dimension::Fadd1];
            ...
            default: {
                throw psnd_error("Unsupport Sampling for SQC");
            }
        }
        for (int i = 0; i < fdim; ++i) {
            psnd_real randu;
                ...
                for (int k = 0, kk = 0; k < Dimension::F; ++k, kk += Dimension::Fadd1) {
                    ...
                    switch (sqc_type) {
                        ...
                        case ElectronicSamplingPolicy::SQCsqr:  // @bug?
                            ...
                            break;
                        default: {
                            throw psnd_error("unknown binnig type for sqc");
                        }
                    }
                    if (Outlier) {
                        ker[ij] = phys::math::iz;
